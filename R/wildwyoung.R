...
wyoung <- function(models, param, B, nsims, R = NULL, r = 0, p_val_type = "two-tailed", weights_type = "rademacher", seed = NULL, engine = "R", nthreads = 1, ...){
  ...
  #' @param nsims The number of Monte Carlo Iterations
  ...
  check_arg(nsims, "integer scalar GT{99}")
    ...
    lapply(seq_along(models),
           function(x){
             ...
             if(!is.null(clustid)){
               ...
               pvals <- get_pvals(x = t_boot, ssc = N-k-1)
               pvals
             } else {
               t_boot <-
                   boottest(
  ...
  # start the westfall-young procedure

  # order the p-values, small to large
  pvals_order <- order(pvals)
  pvals <- pvals[pvals_order]
  # sort the bootstrap pvals
  boot_pvals <- boot_pvals[,pvals_order]

  boot_qvals <- matrix(NA, B, S)
  for(i in 1:B){

    boot_qvals[i,S] <- boot_pvals[i, S]
    for(j in (S-1):1){
      boot_qvals[i,j] <- min(boot_qvals[i,j+1], boot_pvals[i,j])
    }
  ...
  ps <- rep(NA,S)
  for(s in 1:S){
    ps[s] <- mean(boot_qvals[,s] < pvals[s])
  }
  ps <- colMeans((boot_qvals < pvals))
